...
contract('HelloBlockchain', (accounts) => {
    it('testing ResponseMessage of HelloBlockchain', async () => {
        ...
        var message = 'hello456'
        var callerAccount = accounts[0];
        console.log(await HelloBlockchainInstance.RequestMessage.call())
        await HelloBlockchainInstance.SendRequest(message, {from: callerAccount});
        var returnValue = await HelloBlockchainInstance.RequestMessage.call();
        console.log(returnValue)
        // Write an assertion below to check the return value of RequestMessage.
        assert.equal(message, returnValue, 'A correctness property about RequestMessage of HelloBlockchain');
    });
